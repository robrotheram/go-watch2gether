...
import { PlayerContext, VolumeContext } from "./Provider";


const Switch = () => {
    const { showVideo, test } = useContext(PlayerContext)
    return (
        <div className="text-gray-300 flex flex-row w-48 gap-1">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" value="" className="sr-only peer" checked={showVideo} onClick={()=>{test(!showVideo)}}/>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-movie" width={24} height={24} viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
...
const VolumeControl = React.memo(() => {
    ...
    const MuteBtn = ({ onClick }) => {
        return <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-gray-500 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white mr-1"
            fill="currentColor"
            viewBox="0 0 24 24"
            strokeWidth="2"
            stroke="currentColor"
    ...
    return (
        <>
            <div className="items-center md:flex hidden w-48 gap-1">
...
const Player = ({ state }) => {
    ...
    return (
        <div className="flex-shrink grid w-full h-40 grid-cols-1 px-8 pt-1  md:grid-cols-3 bg-zinc-900">
            <div></div>
            {state.Active ?
