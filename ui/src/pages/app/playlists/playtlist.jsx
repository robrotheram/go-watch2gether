...
const PlaylistHeader = ({ playlist, active, onClick, onUpdate }) => {
  ...
  return (
    <li onClick={onClick} className={`${active && "bg-violet-950"} flex items-center p-2 rounded-lg text-white hover:bg-violet-800`}>
...
export const PlaylistBtn = ({ playlists }) => {
  ...
  const loadPlaylist = async (id) => {
    try {
      ...
      toast.success("Adding playlist to queue");
    } catch (e) {
  ...
  return <div className="w-full md:w-60 px-4 relative flex justify-end" ref={ref}>
    ...
    <div className={`${!show && "hidden"} z-10 absolute bottom-20 w-60 right-4 bg-white rounded-lg dark:bg-violet-950 shadow`}>
      {playlists.length > 0 && <ul className="max-h-60 py-2 overflow-y-auto hide-scrollbar text-gray-700 dark:text-violet-200" aria-labelledby="dropdownUsersButton">
        {playlists.map(playlist => (
          <li key={playlist.id} onClick={() => loadPlaylist(playlist.id)} className="flex cursor-pointer justify-center px-4 py-2 text-center hover:bg-gray-100 dark:hover:bg-violet-600 dark:hover:text-white">
            {playlist.name}

          </li>
        ))}
      </ul>}
      <Link to="playlists" className={`${playlists.length == 0 && "rounded-t-lg"}  flex justify-center p-3 text-sm font-medium text-white border-t border-gray-200 rounded-b-lg bg-gray-50 dark:border-violet-600 hover:bg-gray-100 dark:bg-violet-700 dark:hover:bg-violet-600`}>
        Mange playlists
      </Link>
    </div>
  </div>
